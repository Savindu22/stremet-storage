...
function isStorageAssignment(assignment) {
    return assignment != null && 'shelf_slot_id' in assignment;
}
function isMachineAssignment(assignment) {
...
function getAssignmentLocation(assignment) {
    if (isStorageAssignment(assignment)) {
        return (0, rackCells_1.buildRackLocationCode)(assignment.rack_code, assignment.row_number, assignment.column_number);
    }
 ...
 */
exports.trackingRouter.get('/unit/:unitCode', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { unitCode } = req.params;
    // Check storage assignments (shelf)
    const shelfResult = await pool_1.default.query(`
...
exports.trackingRouter.post('/scan', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        if (isUnit) {
            ...
            const saResult = await client.query(`SELECT sa.*, i.id as item_id, i.weight_kg, i.volume_m3,
                    r.code AS rack_code, ss.row_number, ss.column_number
             FROM storage_assignments sa
             JOIN items i ON sa.item_id = i.id
             JOIN shelf_slots ss ON sa.shelf_slot_id = ss.id
             JOIN racks r ON ss.rack_id = r.id
             WHERE sa.unit_code = $1 AND sa.checked_out_at IS NULL`, [scan_code]);
            const maResult = await client.query('SELECT ma.*, i.id as item_id, i.weight_kg, i.volume_m3, m.code as machine_code FROM machine_assignments ma JOIN items i ON ma.item_id = i.id JOIN machines m ON ma.machine_id = m.id WHERE ma.unit_code = $1 AND ma.removed_at IS NULL', [scan_code]);
        ...
        if (location_code.startsWith('M/')) {
            ...
            if (sourceType === 'machine' && isMachineAssignment(currentAssignment) && currentAssignment.machine_id === machine.id) {
                ...
                await client.query(`INSERT INTO activity_log (id, item_id, action, from_location, to_location, performed_by, notes)
                 VALUES ($1, $2, 'note_added', $3, $4, $5, $6)`, [(0, uuid_1.v4)(), itemId, location_code, location_code, performed_by, notes ? `Status toggled to ${nextStatus}. ${notes}` : `Status toggled to ${nextStatus}`]);
            ...
            if (!isUnit) {
                ...
                if (sourceType === 'shelf' && isStorageAssignment(currentAssignment)) {
                    ...
                    await client.query('UPDATE shelf_slots SET current_count = GREATEST(current_count - 1, 0), current_weight_kg = GREATEST(current_weight_kg - $1, 0), current_volume_m3 = GREATEST(current_volume_m3 - $2, 0) WHERE id = $3', [unitWeight, unitVolume, currentAssignment.shelf_slot_id]);
                }
            ...
            await client.query(`INSERT INTO machine_assignments (id, item_id, machine_id, unit_code, status, quantity, assigned_at, assigned_by, notes)
             VALUES ($1, $2, $3, $4, $5, $6, NOW(), $7, $8)`, [newMaId, itemId, machine.id, unitCode, 'queued', unitQuantity, performed_by, notes]);
            await client.query(`INSERT INTO activity_log (id, item_id, action, from_location, to_location, performed_by, notes)
             VALUES ($1, $2, 'move', $3, $4, $5, $6)`, [(0, uuid_1.v4)(), itemId, sourceLocation || 'New', location_code, performed_by, notes || 'Started production phase']);
            await client.query('COMMIT');
    ...
    `, [location_code]).catch(() => {
        ...
        if (shelfResult.rows.length > 0) {
            ...
            if ((Number(shelf.current_count) || 0) >= (Number(shelf.capacity) || 0))
                throw new Error('Target shelf slot is full');
            if ((Number(shelf.current_weight_kg) || 0) + unitWeight > (Number(shelf.max_weight_kg) || 0))
                throw new Error('Target shelf slot weight limit exceeded');
            if ((Number(shelf.current_volume_m3) || 0) + unitVolume > (Number(shelf.max_volume_m3) || 0))
                throw new Error('Target shelf slot volume limit exceeded');
            // Checkout from old
            ...
            await client.query(`INSERT INTO storage_assignments (id, item_id, shelf_slot_id, unit_code, quantity, checked_in_at, checked_in_by, notes)
             VALUES ($1, $2, $3, $4, $5, NOW(), $6, $7)`, [newSaId, itemId, shelf.id, unitCode, unitQuantity, performed_by, notes]);
            await client.query('UPDATE shelf_slots SET current_count = current_count + 1, current_weight_kg = current_weight_kg + $1, current_volume_m3 = current_volume_m3 + $2 WHERE id = $3', [unitWeight, unitVolume, shelf.id]);
            await client.query(`INSERT INTO activity_log (id, item_id, action, from_location, to_location, performed_by, notes)
             VALUES ($1, $2, 'move', $3, $4, $5, $6)`, [(0, uuid_1.v4)(), itemId, sourceLocation, location_code, performed_by, notes || 'Moved via scanner']);
            await client.query('COMMIT');
        ...
        if (location_code === 'EXIT') {
            ...
            await client.query(`INSERT INTO activity_log (id, item_id, action, from_location, to_location, performed_by, notes)
             VALUES ($1, $2, 'check_out', $3, 'SHIPPED', $4, $5)`, [(0, uuid_1.v4)(), itemId, sourceLocation, performed_by, notes || 'Final dispatch via scanner']);
            await client.query('COMMIT');
